...
def get_all_planning(projet_id=None, start_date=None, end_date=None):
    ...
    if not df.empty:
        ...
        df.rename(columns={"nom_projet": "Projet", "veh_id": "ID Véhicule", "nom_test": "Nom du Test",
                           "interlocuteur": "Interlocuteur", "duree": "Durée (jours)",
                           "sopm": "Date SOPM", "lrm": "Date LRM"}, inplace=True)
    return df
...
# -------------------------------
    ...
    if not projets.empty:
        ...
        if not df.empty:
            ...
            fig = px.timeline(df, x_start="Date Début", x_end="Date Fin", y="ID Véhicule",
                              color="Nom du Test", text="Nom du Test",
        ...
        if st.button("Supprimer véhicule"):
            veh_db_id = vehicules_existants_query.loc[
                vehicules_existants_query["veh_id"] == vehicule_suppr, "veh_db_id"
            ].iloc[0]
