...
def sauvegarder_projet(nom, vehicules, essais):
    os.makedirs("projets", exist_ok=True)
    with open(f"projets/{nom}.json", "w") as f:
        json.dump({"vehicules": vehicules, "essais": essais}, f)

if st.sidebar.button("💾 Enregistrer le projet"):
    sauvegarder_projet(nom_projet, vehicules, essais)
    st.sidebar.success(f"Projet '{nom_projet}' enregistré avec succès !")
...
if st.button("📅 Générer le planning"):
    ...
    for veh in vehicules:
        date_courante = datetime.fromisoformat(veh["sopm"]).date()
        for test in essais:
            date_debut = date_courante
            date_fin = date_debut + timedelta(days=test["duree"] - 1)
            semaine = date_debut.isocalendar()[1]
            sopm_date = datetime.fromisoformat(veh["sopm"]).date()
            lrm_date = datetime.fromisoformat(veh["lrm"]).date()
            ...
            planning.append({
                ...
                "Semaine": semaine,
                "Date SOPM": f"{sopm_date} {alerte_sopm}",
                "Date LRM": f"{lrm_date} {alerte_lrm}",
